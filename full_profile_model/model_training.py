...
# ==============================================================================
# 1.5 FLUX PROFILE-BASED FEATURE ENGINEERING (CLEAN & OVERFLOW-PROOF)
# ==============================================================================
print("Engineering features from flux profiles (domain-informed, no leakage)...")

flux_mean = np.mean(X, axis=1)
flux_std = np.std(X, axis=1)
flux_min = np.min(X, axis=1)
flux_max = np.max(X, axis=1)
flux_range = flux_max - flux_min
flux_median = np.median(X, axis=1)
flux_q25 = np.percentile(X, 25, axis=1)
flux_q75 = np.percentile(X, 75, axis=1)
flux_peak_idx = np.argmax(X, axis=1) / X.shape[1] 
flux_peak_value = flux_max

# We temporarily shift X up so it is strictly positive for Centroid & Entropy math
X_shifted = X - np.min(X, axis=1, keepdims=True) + 1e-10

freq_indices = np.arange(X.shape[1])
flux_centroid = np.sum(X_shifted * freq_indices[np.newaxis, :], axis=1) / np.sum(X_shifted, axis=1)

# Because X is already log-scaled, Skewness & Kurtosis will not hit Infinity
flux_skewness = skew(X, axis=1)
flux_kurtosis = kurtosis(X, axis=1)
...
flux_slope = np.mean(np.diff(X, axis=1), axis=1)
flux_curvature = np.std(np.diff(X, axis=1), axis=1)
...
feature_names = ['flux_mean', 'flux_std', 'flux_min', 'flux_max', 'flux_range', 'flux_median',
                 'flux_q25', 'flux_q75', 'flux_peak_idx', 'flux_peak_value', 'flux_centroid',
                 'flux_skewness', 'flux_kurtosis', 'flux_entropy', 'flux_slope', 'flux_curvature']

X_combined = np.hstack([X, engineered_features])

# Final safety sweep: catch any weird math artifacts before they hit the model
X_combined = np.nan_to_num(X_combined, nan=0.0, posinf=0.0, neginf=0.0)

print(f"  ✓ Engineered {engineered_features.shape[1]} flux-profile-based features")
print(f"  ✓ Feature names: {feature_names}")
print(f"  ✓ Combined feature shape: {X_combined.shape}")

# Use combined features for all downstream analysis
X = X_combined


# ==============================================================================
# 2. DATA SCALING (CRITICAL FOR ML)
# ==============================================================================
print("Scaling features and applying physics transformations...")

# Scale X (Flux profiles)
X_scaler = StandardScaler()
X_scaled = X_scaler.fit_transform(X)

# PHYSICS TRANSFORMS ON TARGET VARIABLES (y) BEFORE SCALING:
...
# Scale y (Forces [log_M_dot, h_gamma, Gamma, theta_deg] strictly between 0 and 1)
...
plt.tight_layout()
plt.savefig("learningcurve1old.png", dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig("resultsrun1old.png", dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig("freqrun1old.png", dpi=300, bbox_inches='tight')
plt.show()
...
joblib.dump(reg_xgb, r"C:\Users\kzore\Desktop\trained_xgb_model1old.joblib")

# Save the scalers (THIS IS CRITICAL!)
joblib.dump(X_scaler, r"C:\Users\kzore\Desktop\X_scaler1old.joblib")
joblib.dump(y_scaler, r"C:\Users\kzore\Desktop\y_scaler1old.joblib")
