...
# ==============================================================================
# 3. CHECKPOINT & SAVE SYSTEM (defined before main loop)
# ==============================================================================
def save_checkpoint(checkpoint_num):
    ...
    if checkpoint_num == 1:
        # First checkpoint: create new file
        with h5py.File(desktop_path, "w") as hf:
            hf.create_dataset("X_flux_profiles", data=X_array, maxshape=(None, 7000))
        ...
        # Subsequent checkpoints: append to existing file
        with h5py.File(desktop_path, "a") as hf:
            total_samples = len(X_features)
            
            # RESIZE BOTH DATASETS! (This was the missing piece)
            hf["X_flux_profiles"].resize((total_samples, 7000))
            hf["y_parameters"].resize((total_samples, 4)) 
            
            # Write only the new batch
...
# ==============================================================================
num_samples = 100
target_freq1 = 1e7      # 1 GHz observation
target_freq2 = 1e8      # 1 GHz observation
target_freq3 = 1e9      # 1 GHz observation
target_freq4 = 1e10      # 1 GHz observation
target_freq5 = 1e11     # 1 GHz observation
target_freq6 = 1e12      # 1 GHz observation
target_freq7 = 1e13      # 1 GHz observation
...
desktop_path = r"C:\Users\kzore\Desktop\jet_training_data.TEST100.h5"
checkpoint_interval = 500  # Save every 500 samples
checkpoint_counter = 0

print(f"Starting generation of {num_samples} synthetic jet observations...")
print(f"Progress will be saved every {checkpoint_interval} samples\n")

for i in tqdm(range(num_samples)):
    try:
        # 1. Randomize underlying stable variables
        ...
        # 2. CALCULATE the physical parameters safely
        ...
        # NEW: Lock the magnetic field to exactly 1% of thermal pressure
        b_fraction = 0.01
        rand_b0 = np.sqrt(b_fraction * 8.0 * np.pi * 3.0 * rand_p0)
        
        # NEW: Lock the Pressure profile to exactly -2.0 (standard conical jet)
        def pressure_law(z):
            return rand_p0 * (z / Z_0)**(-2.0)

        # 4. Initialize Physics Engine (Make sure to pass theta_deg!)
        ...
        for z in z_values:
            ...
            if data and 'n_c_list_thita1' in data and len(data['n_c_list_thita1']) > 0:
                ...
                # REMOVED the strict IF statement. 
                # We let np.interp handle out-of-bounds frequencies by setting them to 0.0
                val = np.interp(
                    [target_freq1, target_freq2, target_freq3, target_freq4, target_freq5, target_freq6, target_freq7], 
                    freqs, 
                    fluxes,
                    left=0.0,   # If target freq is lower than jet's min emission, flux = 0
                    right=0.0   # If target freq is higher than jet's max emission, flux = 0
                )
                
                flux_profile.extend(val)
            else:
                # If physics breaks completely, mark sample as invalid
                freq_coverage_valid = False
                break
        
        # 6. Quality Check
        flux_profile = np.array(flux_profile)
        # The AI will now predict Mass Rate, Enthalpy, Velocity, and Viewing Angle!
        if (freq_coverage_valid and len(flux_profile) == 7000 and 
            not np.isnan(flux_profile).any() and np.max(flux_profile) > 0):
            X_features.append(flux_profile)
...
# Final save (catch any remaining samples not saved by checkpoint)
